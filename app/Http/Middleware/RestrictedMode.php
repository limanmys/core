...
class RestrictedMode
{
    ...
    {
        $safeRoutes = [
            "extension_server", "login", "extension_server_settings_page", "extension_server_settings", "logout", "password_change", "password_change_save"
        ];
        if(env('LIMAN_RESTRICTED') == true && user() && !user()->isAdmin()){
            $request->request->add([
                ...
                "extension" => \App\Extension::find(env("LIMAN_RESTRICTED_EXTENSION"))
            ]);
