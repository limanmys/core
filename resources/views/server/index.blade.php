...
@section('content')
            ...
            <div class="card">
                <div class="card-body">
                    @if (\App\Models\Permission::can(user()->id, 'liman', 'id', 'add_server'))
                        <button href="#tab_1" type="button" class="btn btn-success" data-toggle="modal"
                            data-target="#add_server"><i class="nav-icon fas fa-plus mr-1"></i>
                            {{ __('Sunucu Ekle') }}</button><br><br>
                    @endif
                    ...
                    @include('table', [
                        'value' => $servers,
                        'title' => ['Sunucu Adı', 'İp Adresi', '*hidden*', 'Kontrol Portu', 'Eklenti Sayısı', '*hidden*', '*hidden*'],
                        'display' => ['name', 'ip_address', 'type:type', 'control_port', 'extension_count', 'city:city', 'id:server_id'],
                        'menu' => [
                            'Düzenle' => [
                                'target' => 'edit',
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                ...
                <ul class="nav nav-tabs" role="tablist" style="padding:20px;">
                    <li class="nav-item">
                        <a class="nav-link active" data-toggle="pill" id="networkTab" href="#network" role="tab"
                            aria-controls="network" aria-selected="true">{{ __('Bağlantı Bilgileri') }}</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="pill" id="generalTab" href="#general" role="tab"
                            aria-controls="general">{{ __('Genel Ayarlar') }}</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="pill" id="keyTab" href="#key" role="tab"
                            aria-controls="key">{{ __('Anahtar') }}</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="pill" id="summaryTab" onclick="setSummary()" href="#summary"
                            role="tab" aria-controls="summary">{{ __('Özet') }}</a>
                    </li>
                </ul>
                <div class="tab-content">
                    <div class="tab-pane fade show active" id="network" role="tabpanel" aria-labelledby="networkTab">
                        <form onsubmit="return checkAccess(this)">
                            <div class="modal-body">
                                ...
                                <input type="text" id="serverHostName" name="hostname" class="form-control"
                                    placeholder="{{ __('Sunucunuzun Hostname yada IP Adresini girin.') }}" required><br>
                                <label class="text-md">{{ __('Sunucunuzun Portu') }}</label>
                                <div>
                                    {{ __('Sunucunuzun açık olup olmadığını algılamak için kontrol edilebilecek bir port girin.') }}
                                </div>
                                <pre>{{ __("SSH : 22\nWinRM : 5986\nActive Directory, Samba : 636") }}</pre>
                                <input id="serverControlPort" type="number" name="port" class="form-control"
                                    placeholder="{{ __('Kontrol Portu Girin (Yalnızca Sayı).') }}" required min="-1">
                                <small><i>{{ __('Eğer hedefiniz UDP protokolü üzerinden dinliyorsa bu kontrolü atlamak için -1 girebilirsiniz.') }}</i></small>
                            </div>
                            <div class="modal-footer">
                                <button type="submit" class="btn btn-primary"><i class="fas fa-plug"></i>
                                    {{ __('Bağlantıyı Kontrol Et') }}</button>
                            </div>
                        </form>
                    </div>
                    <div class="tab-pane fade" id="general" role="tabpanel" aria-labelledby="generalTab">
                        <form onsubmit="return checkGeneral(this)">
                                    ...
                                    <div class="radio">
                                        <label style="font-weight: 400; margin-right: 10px;">
                                            <input type="radio" name="operating_system" value="windows"
                                                data-content="{{ __('Microsoft Windows') }}">
                                            {{ __('Microsoft Windows') }}
                                        </label>
                                        <label style="font-weight: 400">
                                            <input type="radio" name="operating_system" value="linux" checked
                                                data-content="{{ __('GNU/Linux') }}">
                                            {{ __('GNU/Linux') }}
                                        </label>
                                    </div>
                                </div>
                            ...
                            <div class="modal-footer">
                                <button type="submit" class="btn btn-primary"><i class="fas fa-wrench"></i>
                    ...
                    <div class="tab-pane fade" id="summary" role="tabpanel" aria-labelledby="summaryTab">
                        <div class="modal-body">
                            ...
                            <table class="notDataTable">
                                ...
                                <tr>
                                    <td>{{ __('Anahtar Paylaşımı') }}</td>
                                    <td id="tableShared"></td>
                                </tr>
                            </table>
                        </div>
                        <div class="modal-footer">
                            <button type="button" onclick="addServer()" class="btn btn-success"><i
                                    class="fas fa-check"></i> {{ __('Sunucuyu Ekle') }}</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function setSummary() {
            ...
            $("#tableShared").text(($("#sharedKey").is(':checked') === true) ? "{{ __('Paylaşımlı') }}" :
                "{{ __('Paylaşımsız') }}");
        }
