<?php
    ...
    $search = ldap_search($ldap_connection, "dc=ldap,dc=lab" , "(objectclass=device)", ["dn"]);
    $computers = ldap_get_entries($ldap_connection, $search);
    $mert = [];
    $user_details = [];
    for($i = 0 ; $i < $computers["count"] ; $i++){
        $computer = $computers[$i]["dn"];
        $arr = explode(",", $computer);
