...
function renderNotifications(data, type, target, exclude) {
  ...
  data.forEach((notification) => {
    element.append([...$("<div />").addClass("dropdown-divider").append("<a />").find("a").addClass("dropdown-item").attr("href", `/bildirim/${notification["id"]}`).append("<span />").find("span").css("color", ["error", "health_problem"].includes(notification["type"]) ? "#f56954" : "#00a65a").css("width", "100%").text(notification["title"]).parents()].reverse())
    var displayedNots = [];
