...
def before():
    ...
    output, streams, had_errors = client.execute_ps("foreach ($file in Get-Item -Path WSMan:\\localhost\\ClientCertificate\\*) { Where-Object { $file.Keys | Select-Item \"administrator\"} ")
    if (output != ""):
    ...
    output1, streams, had_errors = client.execute_ps(
       "Get-ChildItem -Path cert:\LocalMachine\\root | Where-Object { $_.Subject -eq \"CN=%s\"}" % userName)
    if (output1 != ""):
    ...
    output2, streams, had_errors = client.execute_ps(
       "Get-ChildItem -Path cert:\LocalMachine\\trustedPeople | Where-Object { $_.Subject -eq \"CN=%s\"}" % userName)
    if (output2 != ""):
...
def run():
    ...
    try:
        ...
        """ % (userName, userPassword, userName, userName))
        if had_errors:
            raise Exception()
    except:
